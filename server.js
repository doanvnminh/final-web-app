...
app.use(async (req, res, next) => {
    ...
    if (req.method === 'GET' && !req.path.startsWith('/css') && !req.path.startsWith('/images') && !req.path.startsWith('/api')) {
...
// The Login Guard (Must be logged in)
...
app.get('/house/:id', async (req, res) => {
    try {
        ...
        res.status(404).send('Lỗi ID không hợp lệ');
    }
});


app.post('/api/houses/:id/comment', express.json(), async (req, res) => {
    try {
...
app.post('/contact', async (req, res) => {
    try {
        ...
        if (req.headers['content-type'] === 'application/json') {
            return res.json({ success: true, message: 'Đã gửi tin nhắn thành công!' });
        }
